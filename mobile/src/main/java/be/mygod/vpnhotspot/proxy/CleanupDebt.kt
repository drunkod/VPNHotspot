...
// CleanupAccumulator
//
// Applies a timeout to each step, attempts all eligible steps, records per-
// resource resolution, and returns itemized debt. Never marks a resource
// resolved because another resource succeeded.
// ---------------------------------------------------------------------------
...
class CleanupAccumulator(private val reporter: ProxyErrorReporter? = null) {
    ...
    suspend fun stepSucceeded(name: String, block: suspend () -> CleanupReport): Boolean {
        return try {
            ...
            reporter?.report("proxy.cleanup.$name", t, emptyList())
            false
        }
    }
